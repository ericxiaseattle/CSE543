...
It also assumes the interpolation condition, satisfied by many overparameterized deep NNs achieveing zero loss on training data:

\begin{definition}[Interpolation Condition]
	We say that the interpolation condition holds if there exists $x^*\in\mathcal{X}^*$ s.t.
	\[
		\min_{x\in\mathbb{R}^n}f_i(x) = f_i(x^*)\ \ \text{for}\ \ i=1, \dots, n.
	\]
\end{definition}
...
\begin{lemma}[Variance Bound]
	If $g\in\text{ER}(\rho)$ then

	\[
		\mathbb{E}\left[||g(x)||^2\right]\leq 4\rho(f(x) - f^*) + ||\nabla f(x)||^2 + 2\sigma^2.
	\]
\end{lemma}

This bound allows the avoidance of the stronger assumptions of bounded gradient or bounded variance (\textit{General Assumption 5}).
...
With only \textit{General Assumptions}, Definition 3.1 (ER) is shown to be strictly weaker than multiple previously used
assumptions, including the important
